...
class Trained_Agent:
    def __init__(self, environment, epsilon, gamma, alpha, agent, decaying_eps):
    ...
    def create_q_table(self):
        self.q_table = np.zeros(
            (
                len(self.environment.get_state_space().keys()),
    ...
    def get_training_info(
        ...
        if num_episode % 5000 == 0:
            if self.agent == "Q":
                ...
                agent = "Sarsa"
            print(
                f"Training info of {agent}: episode {num_episode}, episode reward:{ep_rewards}, number of steps: {number_of_iterations}, epsilon: {self.epsilon}, gamma: {self.gamma}, alpha: {self.alpha} starting position: {starting_position}, trajectory: {trajectory}, {info}"
            )
    ...
    def plot_training_results(self,ep_results):
        ...
        if self.agent == "Q":
            plt.title("Average rewards per episode, Q-learning")
        else:
            plt.title("Average rewards per episode, Sarsa")
        plt.gcf()
        plt.show()
    ...
    def train_agent(self, num_games,q_table,max_steps):
        ...
        self.plot_training_results(ep_results)
