...
parser.add_argument('keyword', type=str,
                    help='enter one of the keywords: train, play, ' +
                     'compare_algorithms, compare_envs, compare_epsilon')
parser.add_argument('num_eps', type=int,
                    help='enter the number of episodes')
parser.add_argument('--algorithm', type=str, default="Q",
                    help='enter the chosen algorithm Q for Q-learning ' +
                     'or S for Sarsa, (example: %(default)s)')
parser.add_argument('--q_table', type=str, default="",
                    help='enter the path for saving the Q-table ' +
                    '(example: q_tables/Q_learning_Q-table.pkl')
parser.add_argument('--env_size', type=int, default=7,
                    help='enter the size of the grid (default: %(default)s) ' +
                     'Note: when playing the game, the environment size should' +
                     'correspond to the size used while training the agent.')
parser.add_argument('--epsilon', type=float, default=1,
                    help='enter the value of starting epsilon ' +
                    '(default: %(default)s)')
parser.add_argument('--decaying_eps', default='True',
                    help='enter the True for e-greedy policy, otherwise the ' +
                    'epsilon value will be fixed (default: %(default)s)')
...
parser.add_argument('--max_steps', type=int, default=100,
                    help='enter the maximum number of steps that '
                     'agent can take per episode (default: %(default)s)')
...
def check_decaying_eps(decaying_eps):
    if decaying_eps != 'True' and decaying_eps != 'False':
        parser.error("Incorrect argument given for --decaying_eps." +
