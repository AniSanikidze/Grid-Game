...
def env_comparison(algorithm,epsilon,gamma,alpha,
                   decaying_eps,num_episodes,max_steps):
    env_small = Grid(3)
    env_large = Grid(7)

    small_grid_training = Training(env_small,epsilon,gamma,
                                   alpha,algorithm,decaying_eps)
    ...
    plt.plot(small_grid_training_results['ep'],
             small_grid_training_results['avg'],
             marker=".", label="3x3 Grid")
    plt.plot(large_grid_training_results['ep'],
             large_grid_training_results['avg'],
             marker=".", label="7x7 Grid")
